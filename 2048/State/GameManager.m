...
- (void)moveToDirection:(MyDirection)direction
{
    ...
    if (direction == MyDirectionUp || direction == MyDirectionDown) {
        [_grid forEach:^(Position position) {
            if ((tile = [_grid tileAtPosition:position])) {
                ...
                for (NSInteger i = position.x + unit; iterate(i, reverse, _grid.dimension, -1); i += unit) {
                    MyTile *t = [_grid tileAtPosition:PositionMake(i, position.y)];
                    ...
                    // Try to merge to the tile in the cell.
                        ...
                        if (GSTATE.gameType == GameTypePowerOf3) {
                            Position further = PositionMake(i + unit, position.y);
                ...
                if (target != position.x) {
                    [tile moveToCell:[_grid cellAtPosition:PositionMake(target, position.y)]];
                    _pendingScore++;
                }
            }
        ...
        [_grid forEach:^(Position position) {
            if ((tile = [_grid tileAtPosition:position])) {
                ...
                for (NSInteger i = position.y + unit; iterate(i, reverse, _grid.dimension, -1); i += unit) {
                    MyTile *t = [_grid tileAtPosition:PositionMake(position.x, i)];
                    ...
                    if (!t) target = i;
                        ...
                        if (GSTATE.gameType == GameTypePowerOf3) {
                            Position further = PositionMake(position.x, i + unit);
                            MyTile *ft = [_grid tileAtPosition:further];
                            if (ft) {
                                level = [tile merge3ToTile:t andTile:ft];
                            }
                ...
                if (target != position.y) {
                    [tile moveToCell:[_grid cellAtPosition:PositionMake(position.x, target)]];
                    _pendingScore++;
                }
            }
        } reverseOrder:reverse];
...
- (BOOL)adjacentMatchesAvailable
{
    for (NSInteger i = 0; i < _grid.dimension; i++) {
        for (NSInteger j = 0; j < _grid.dimension; j++) {
            ...
            MyTile *tile = [_grid tileAtPosition:PositionMake(i, j)];
            ...
            if (GSTATE.gameType == GameTypePowerOf3) {
                if (([tile canMergeWithTile:[_grid tileAtPosition:PositionMake(i + 1, j)]] &&
                     [tile canMergeWithTile:[_grid tileAtPosition:PositionMake(i + 2, j)]]) ||
                    ([tile canMergeWithTile:[_grid tileAtPosition:PositionMake(i, j + 1)]] &&
                     [tile canMergeWithTile:[_grid tileAtPosition:PositionMake(i, j + 2)]])) {
                        return YES;
                    }
            } else {
                if ([tile canMergeWithTile:[_grid tileAtPosition:PositionMake(i + 1, j)]] ||
