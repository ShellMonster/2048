...
- (instancetype)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) _manager = [[GameManager alloc] init];
    return self;
}
...
- (void)loadBoardWithGrid:(MyGrid *)grid {
    ...
    UIImage *image = [GridView gridImageWithGrid:grid];
    ...
    [_board setScale:1/[UIScreen mainScreen].scale]; //This solves the Scaling problem in 6 Plus and 6S Plus
...
- (void)commitTranslation:(CGPoint)translation {
    ...
    if (absX > absY * VALID_SWIPE_DIRECTION_THRESHOLD) {
        translation.x < 0 ? [_manager moveToDirection:MyDirectionLeft] :
        [_manager moveToDirection:MyDirectionRight];
    } else if (absY > absX * VALID_SWIPE_DIRECTION_THRESHOLD) {
        translation.y < 0 ? [_manager moveToDirection:MyDirectionUp] :
        [_manager moveToDirection:MyDirectionDown];
    }
